...
import { gameConfParserSwitcher } from './services/utils.js';
import { wait } from '../../services/utils.js';
import saveDataProgression from './services/parser-backup.js';
...
function getItems(category, links, game, newPage) {
	//currentPosition = currentPosition + 48;
	//console.log(game)
	...
	return new Promise(async function (resolve, reject) {
		while (currentPosition < links.length) {
			try {
				...
				const item = await getData(links[currentPosition], newPage);
				//console.log(item)
...
function getData(url, newPage) {
	...
	gParserList.categories.forEach(item => {
		try {
			const regex = new RegExp('\\b(' + item.lang.en + '|' + item.lang.fr + '|' + item.lang.es + ')\\b');
			if (regex.test(url)) categoryPromise = item.function(url, newPage);
		} catch (error) {
