...
async function getPets(url, pageNew) {
	...
	if(test){return {statusCode:'404'}}
		...
		const haveSet = await pageNew.$eval('body > div.ak-mobile-menu-scroller > div.container.ak-main-container > div > div > div > main > div.ak-container.ak-main-center > div > div:nth-child(4) > div.ak-panel-title > a', () => true).catch(() => false);
		...
		//es parte de un set?
		if(haveSet){
			pet.setId = cheeriParser('body > div.ak-mobile-menu-scroller > div.container.ak-main-container > div > div > div > main > div.ak-container.ak-main-center > div > div:nth-child(4) > div.ak-panel-title').find('a').attr('href').replace(/\D/g, '')
		}
