		...
		if (typeof $akContainer.eq(1) !== 'undefined') {
			$akContainer.eq(1).find('div.col-sm-6').each(function (i, element) {
				if (cheeriParser(this).find('div.ak-container.ak-panel').eq(1).html() !== null) {
					categorySwitch(cheeriParser(this).find('div.ak-container.ak-panel').eq(1).find('div.ak-panel-title').text().trim().toLowerCase(), cheeriParser(this).find('div.ak-container.ak-panel').eq(1).html());
				}
...
function conditionParse(weapon, body) {
	...
	weapon.conditions = conditionTab.map(function (string) {
