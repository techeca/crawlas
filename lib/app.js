...
(async function main() {
  ...
  try {
    ...
    let cmdResponse = ifResume === true ? await getCategory(ifResume) : JSON.parse(await getCategory(ifResume));
...
async function resumeLastParse() {
    ...
    console.log('games '+resume.game)
    await puppeteer.launch({headless: true})
    .then( async browser => {
      const pageNew = await browser.newPage();
      await pageNew.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64)');
      const parsingInfo = await parserProcess(resume.category, resume.links, resume.game, pageNew);
      parsingFinished(parsingInfo);
    })
	} catch (error) {
